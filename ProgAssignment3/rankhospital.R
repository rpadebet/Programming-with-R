...
rankhospital <- function(state = "NJ",outcome = "heart attack",rank = 1)
{
    ## Read outcome data
    ...
    if(rank=="best") {rank=1} 
        else if(rank =="worst") {rank = max(outcome.use.func$Rank)}
            else {rank=rank}
    
    try (if(rank >max(outcome.use.func$Rank)) stop("NA"))
    
    rank.hospital <- outcome.use.func[outcome.use.func$Rank ==rank,1]
